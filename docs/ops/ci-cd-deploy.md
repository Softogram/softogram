...
A failed build stops the job before anything syncs — a broken build never reaches production.
...
GitHub. The role trust policy only allows assumption from `nomotomo/softogram` on `main`.
